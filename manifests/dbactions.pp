...
define oradb::dbactions(
  ...
  Enum['start', 'stop', 'running', 'abort','mount','upgrade'] $action = 'start',
  ...
  if ( $db_type == 'database' and !($action in ['running','start','abort','stop','mount','upgrade'])) {
    fail('Unrecognized action for db_type grid and asm, use running, start, abort, stop, mount or upgrade')
  }
